...
# .service 'pbRoot', ($pb)-> $pb "http://sparks-development.firebaseio.com"
...
pongular.injector(['app', 'pong-base']).invoke ($watch, $q, fixture, fbRoot, Profile, pick, $promise)->
	$promise (d)-> fbRoot.remove( d.resolve )
	.then ->
		console.log 'all data removed from firebase'
	.then ->
		console.log 'adding Profiles'
		$q.all (Profile.promise.push(fixture.profile()) for i in [1..3])
	.then (profiles)->
		console.log "#{profiles.length} profiles generated"

		profiles[0].promise.get().then (snap)->
			snap.val().uid
		.then (uid)->
			console.log 'profile uid is', uid
			Profile.promise.get('uid',uid).then (snap)->
				console.log 'found profile:', snap.val()
